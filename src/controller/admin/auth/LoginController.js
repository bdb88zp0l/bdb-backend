...
exports.login = catchAsync(async (req, res) => {
  ...
  const user = await User.findOne({
    ...
    path: "role",
    populate: {
      path: "permissions", // This will populate the permissions inside each role
      model: "Permission", // This specifies the model for the permissions
    },
  });
  if (!user) {
  ...
  if (user.roleType === "superAdmin") {
    ...
    hasDashboardAccess = user?.role.permissions.some(
...
exports.verifyLoginWithOTP = catchAsync(async (req, res) => {
  ...
  try {
    ...
    // Find the user
    const user = await User.findById(authenticatorVerificationId).populate({
      path: "role",
      populate: {
        path: "permissions", // This will populate the permissions inside each role
        model: "Permission", // This specifies the model for the permissions
